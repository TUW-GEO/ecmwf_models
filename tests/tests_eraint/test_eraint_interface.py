...
from ecmwf_models.erainterim.interface import ERAIntGrbImg, ERAIntNcImg, ERAIntGrbDs, ERAIntNcDs
import numpy as np
...
def test_ERAInt_nc_image():
    ...
    nptest.assert_allclose(data.data['swvl1'][100, 64], 0.17185, rtol=1e-4)
    nptest.assert_allclose(data.data['swvl2'][100, 64], 0.179816, rtol=1e-4)
    # data over water
...
def test_ERAInt_nc_image_1d():
    ...
    nptest.assert_allclose(data.data['swvl1'][100 * 480 + 64], 0.17185, rtol=1e-4)
    nptest.assert_allclose(data.data['swvl2'][100 * 480 + 64], 0.179816, rtol=1e-4)
    # data over water
...
def test_ERAInt_grb_image():
    ...
    nptest.assert_allclose(data.data['swvl2'][34, 23], 0.30944514274)
    nptest.assert_allclose(data.lon[34, 23], 16.17183561643, rtol=1e-5)
    nptest.assert_allclose(data.lat[34, 23], 65.6137949705, rtol=1e-5)
    # data over water
    ...
    nptest.assert_allclose(data.lon[0, 0], 0.0)
    nptest.assert_allclose(data.lon[0, 256],  179.99956164383)  # middle of image
...
def test_ERAInt_grb_image_1d():
    ...
    # data over land
    nptest.assert_allclose(data.lon[34 * 512 + 23], 16.17183561643, rtol=1e-5)
    nptest.assert_allclose(data.lat[34 * 512 + 23], 65.6137949705, rtol=1e-5)
    nptest.assert_allclose(data.data['swvl1'][34 * 512 + 23], 0.30950284004211426, rtol=1e-5)
    nptest.assert_allclose(data.data['swvl2'][34 * 512 + 23], 0.30944514274, rtol=1e-5)
    # data over water
...
def test_ERAInt_grb_ds():
    ...
    ds = ERAIntGrbDs(root_path, parameter=['swvl1', 'swvl2'], array_1D=True,
                   mask_seapoints=True)
